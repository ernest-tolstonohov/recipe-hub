document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Initial render via JS so onclick works from the start ─
    const initialData = document.getElementById('recipes-data');
    if (initialData) {
        const initial = JSON.parse(initialData.dataset.recipes || '[]');
        if (initial.length > 0) renderCards(initial, false);
    }
