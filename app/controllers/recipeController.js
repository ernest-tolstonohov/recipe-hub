...
class RecipeController {
    ...
    static async create(req, res) {
        ...
        const image_url = req.file
            ...
            : (req.body.image_url_input?.trim() || null);
    ...
    static async update(req, res) {
        ...
        const image_url = req.file
            ? `/media/${req.file.filename}`
            : (req.body.image_url_input?.trim() || existing_image_url || null);
        const recipeId = req.params.id;

        if (title && title.length > 150) return res.status(400).render('error', { message: 'Title is too long.' });
        if (isNaN(parseInt(prep_time)) || isNaN(parseInt(cook_time)) || isNaN(parseInt(servings)) || parseInt(prep_time) < 0 || parseInt(cook_time) < 0 || parseInt(servings) < 1) {
